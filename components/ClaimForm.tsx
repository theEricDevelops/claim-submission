...
export default function ClaimForm() {
  ...
  const insuredKeyRef = useRef(1)
  const [namedInsureds, setNamedInsureds] = useState<NamedInsured[]>([emptyInsured(0)])
  ...
  function resetForm() {
    ...
    setNamedInsureds([emptyInsured(0)])
    insuredKeyRef.current = 1
    setAdjFirstName('')
    setAdjLastName('')
    setAdjEmail('')
    setAdjPhone('')
  ...
  const addInsured = useCallback(() => {
    ...
    const key = insuredKeyRef.current++
    setNamedInsureds((prev) => [...prev, emptyInsured(key)])
      ...
      case 2:
        return templateFields
          .filter(
            (f) =>
              f.required &&
              !AUTO_POPULATED_FIELDS.has(f.name) &&
  ...
  async function handleNext() {
    ...
    if (currentStep === 0 && state) {
      ...
      try {
        ...
        setTemplateError(err instanceof Error ? err.message : 'Failed to validate template')
        setLoadingTemplates(false)
        return
      }
      setLoadingTemplates(false)
    ...
    if (currentStep === 1) {
      setNamedInsureds((prev) => prev.map((ni) => ({ ...ni, phone: formatPhone(ni.phone) })))
      setLoadingTemplates(true)
      try {
        const res = await fetch('/api/templates/fields', {
          method: 'POST',
        ...
        setTemplateError(err instanceof Error ? err.message : 'Failed to fetch template fields')
      } finally {
        setLoadingTemplates(false)
      }
      return
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    for (const tf of templateFields) {
      ...
      else if (tf.name === 'Insured Mailing Address')
        allValues[tf.name] =
          namedInsureds[0]?.mailingAddress?.formatted || propertyAddress.formatted
      else if (tf.name === 'Second Insured Name')
        allValues[tf.name] = namedInsureds[1] ? insuredName(namedInsureds[1]) : ''
      else if (tf.name === 'Second Insured Phone')
        allValues[tf.name] = namedInsureds[1]?.phone || ''
      else if (tf.name === 'Second Insured Email')
        allValues[tf.name] = namedInsureds[1]?.email || ''
      else if (tf.name === 'Public Adjuster Name')
        allValues[tf.name] = [adjFirstName, adjLastName].filter(Boolean).join(' ')
      ...
      else if (tf.name === 'Public Adjuster Mailing Address')
        allValues[tf.name] = adjMailingAddress.formatted || ''
    }
      ...
      <fieldset ref={stepRef}>
        ...
        {templateError && (
          <div className="warning-msg">
            No DocuSeal template configured for {STATE_NAMES[state] || state}. Create a template
            with name starting with &ldquo;{state}&rdquo;
          </div>
        )}
      </fieldset>
    )
  ...
  function renderStep1() {
    return (
      <fieldset ref={stepRef}>
        ...
        {templateError && <div className="warning-msg">{templateError}</div>}
        {namedInsureds.map((ni, i) => {
          return (
            <div key={ni._key} className="insured-entry">
              ...
              <div className="field-row">
                <div className="field field-narrow">
                  <label htmlFor={`${formId}-type-${i}`}>Type *</label>
                  <select
                    id={`${formId}-type-${i}`}
              ...
              {ni.typeChosen && ni.type === 'individual' && (
                <>
                  <div className="field-row">
                    <div className="field field-narrow">
                      <label htmlFor={`${formId}-salutation-${i}`}>Salutation</label>
                      <select
                        id={`${formId}-salutation-${i}`}
                        value={ni.salutation}
                        onChange={(e) => updateInsured(i, { salutation: e.target.value })}
                      >
                        <option value="">--</option>
                        {SALUTATIONS.map((s) => (
                    ...
                    />
                    <div className="field field-narrow">
                      <label htmlFor={`${formId}-suffix-${i}`}>Suffix</label>
                      <select
                        id={`${formId}-suffix-${i}`}
                        value={ni.suffix}
                        onChange={(e) => updateInsured(i, { suffix: e.target.value })}
                      >
                        <option value="">--</option>
                        {NAME_SUFFIXES.map((s) => (
                          <option key={s} value={s}>
                ...
                <div className="field-checkbox">
                  <label>
                    <input
                      ...
                      onChange={(e) =>
                        updateInsured(i, { differentMailingAddress: e.target.checked })
                      }
                    />
        ...
        {namedInsureds.length < 2 && (
          <button type="button" className="btn-add" onClick={addInsured} aria-label="Add insured">
            +
          </button>
        )}
      </fieldset>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFieldValues((prev) => {
                          ...
                          if (e.target.checked && f.name === 'Emergency Claim')
                            next['Non-Emergency Claim'] = ''
                          if (e.target.checked && f.name === 'Non-Emergency Claim')
                            next['Emergency Claim'] = ''
                          return next
                        })
                      }
                    />
  ...
  function renderStep3() {
    return (
      <fieldset ref={stepRef}>
        ...
        <AddressInput
          label="Public Adjuster Mailing Address"
          value={adjMailingAddress}
          onChange={setAdjMailingAddress}
        />
      </fieldset>
    )
  }
  ...
  function renderStep4() {
    const namedInsuredLabel = (ni: NamedInsured): string => {
      if (ni.type === 'individual') {
        return (
          [ni.salutation, ni.firstName, ni.middleName, ni.lastName, ni.suffix]
            .filter(Boolean)
    ...
    return (
      <fieldset ref={stepRef}>
        ...
        <div className="review-section">
          ...
          {namedInsureds.map((ni, i) => {
            return (
              <div key={ni._key} className="review-item">
                <strong>{i === 0 ? 'First' : `Additional ${i + 1}`}:</strong>{' '}
                {namedInsuredLabel(ni)} &lt;{ni.email}&gt; {ni.phone}
                {ni.differentMailingAddress && (
                  <div className="review-sub">
                    Mailing:{' '}
                    {ni.mailingAddress.formatted ||
                      [
                        ni.mailingAddress.street,
                        ni.mailingAddress.street2,
                        `${ni.mailingAddress.city}, ${ni.mailingAddress.state} ${ni.mailingAddress.zip}`,
                      ]
                        .filter(Boolean)
                        .join(', ')}
                  </div>
        ...
        <div className="review-section">
          ...
          <p>
            {adjFirstName} {adjLastName} &lt;{adjEmail}&gt; {adjPhone} Lic: {adjLicense}
          </p>
  ...
  return (
    <form
      ...
      className="claim-form multi-step"
      onKeyDown={(e) => {
        if (e.key === 'Enter' && currentStep < 4 && !e.shiftKey) {
          e.preventDefault()
          handleNext()
        }
      }}
    >
      ...
      />
      {renderStep()}
      {status.type === 'error' && <div className="error-msg">Error: {status.message}</div>}
      {renderNavButtons()}
    </form>
  )
}
